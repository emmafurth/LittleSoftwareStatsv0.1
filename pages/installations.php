...
$area_chart_data_installs = array();
...
for ( $i = 0; $i < count( $date_range_day ) - 1 ;$i++ ) {
    ...
    $total_for_period = $db->select_events( 'ist', $app_id, $app_ver, $start, $end, true );
    ...
    $area_chart_data_installs[] = $total_for_period;
}
...
// Get execution stats
$total_installs = $db->select_events( 'ist', $app_id, $app_ver, $min_date, $max_date, true );
$period_installs = $db->select_events( 'ist', $app_id, $app_ver, $start_time, $end_time, true );
...
$day_installs_total = $db->select_events( 'ist', $app_id, $app_ver, $date_range_total_day[0], end( $date_range_total_day ), true );
$month_installs_total = $db->select_events( 'ist', $app_id, $app_ver, $date_range_total_month[0], end( $date_range_total_month ), true );

$day_installs = 0;
$month_installs = 0;

if ( count( $date_range_total_day ) - 1 > 0 )
    $day_installs = ( $day_installs_total / ( count( $date_range_total_day ) - 1 ) );
if ( count( $date_range_total_month ) - 1 > 0 )
    $month_installs = ( $month_installs_total / ( count( $date_range_total_month ) - 1 ) );

// Get percentage difference from last month
$last_month = $db->select_events( 'ist', $app_id, $app_ver, $start_time - ( 30 * 24 * 3600 ), $start_time, true );
...
$(document).ready(function() {
        chart = new Highcharts.Chart({
                ...
                series: <?php echo json_encode( array( array( 'name' => 'Installations','data' => $area_chart_data_installs ) ) ); ?>
        });
        ...
        <div class="contentcontainer med left">
            ...
            <div class="contentbox">
                <div>
                    ...
                    <p><span class="total"><?php echo round( $month_installs, 2); ?></span> <?php _e( 'installations per month (average)' ); ?></p>
                    <p><span class="total"><?php echo round( $day_installs, 2); ?></span> <?php _e( 'installations per day (average)' ); ?></p>
                </div>
            </div>
        </div>
        <!-- Overview End -->
