...
class MySQL {
    ...
    /**
     * Selects events using application ID, version, event code, and specified time period
     * @access public
     ...
     * @return int|bool|array Returns list of sessions or number of rows found, otherwise, false if error occurred
     */
    public function select_events( $event_code, $app_id, $app_ver, $start, $end, $count = false, $where = '', $group_by = '' ) {
        if ( !is_numeric( $start ) || !is_numeric( $end ) )
            return false;
        ...
        // Prepare variables
        $event_code = $this->secure_data($event_code);
        $app_id = $this->secure_data($app_id);
        $app_ver = $this->secure_data($app_ver);

        $query = 'SELECT ' . ( ( $count ) ? ( 'COUNT(*) AS count' ) : ( '*' ) ) . ' FROM `' . $this->prefix . 'events` ';
        $query .= "WHERE `EventCode` = '" . $event_code . "' ";
